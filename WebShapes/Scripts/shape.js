...
function attachShapeToNewParrent(event, ui, thisObject) {
	...
	var targetShapeElement = $(event.target); 	// shpesSet
	var draggableSource = ui.helper[0];   // "squareShape"
	...
	var inputWidth = prompt("Enter the shape's primary dimension (width, radius, or base): ");

	// clone element
	var newElement = $(draggableSource).clone();
...
function populateSelection(collectonString) {
	...
	for (i = 0; i < collectionJson.length; i++) {
		...
		var newElement = $(sampleObjectId).clone();
